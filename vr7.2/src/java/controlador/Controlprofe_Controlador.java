        ...
        switch (opcion) {
            case 1: //Agregar Registro
                if (ControlDAO.agregarRegistro()) {
                   request.setAttribute("MensajeExito", "El producto Se prestó al estudiante correctamente");
                   request.getRequestDispatcher("Inventario_Profesor.jsp").forward(request, response);
                   if (DpreDAO.actualizarRegistro()) {                   
                } else {     
                }                
                } else {
                    request.setAttribute("MensajeError", "El producto no pudo ser prestado al estudiante!");  
                    request.getRequestDispatcher("Inventario_Profesor.jsp").forward(request, response);
                }          
            ...
            case 2:
                 if (Codao.actualizarRegistro() ) {       
                    if (DpreDAO.actualizarRegistro()) {  
                         request.setAttribute("MensajeExito", "El producto  fue devulto al profesor");
                      ...
                      request.setAttribute("MensajeError", "El producto no pudo ser devuelto al profesor");
                       request.getRequestDispatcher("Alumnos_pendientes.jsp").forward(request, response);
                 }
